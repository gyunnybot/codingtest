...
class Solution {
    public List<Integer> solution(int[] answers) {
        ...
        for(int i = 0; i < answers.length; i++) {
            if(answers[i] == arr[0].get(i % arr[0].size())) score[0]++;
            if(answers[i] == arr[1].get(i % arr[1].size())) score[1]++;
            if(answers[i] == arr[2].get(i % arr[2].size())) score[2]++;
        }
...
/**
 * 미리 값이 입력되고 변경되지 않으면 [] 배열이 접근이 편한 듯?
 * int[][] arr = {
    {1, 2, 3, 4, 5},
    {2, 1, 2, 3, 2, 4, 2, 5},
    {3, 3, 1, 1, 2, 2, 4, 4, 5, 5}
