...
void snake() {
	...
	while (!dq.empty()) {
		...
		if (!dir_change.empty() && (ret == dir_change.front().first)) { //입력한 시간이 되면 방향을 바꾼다
			dir = (dir + dir_change.front().second) % 4;
			dir_change.pop_front();
		}
	}
...
int main() {
	...
	for (int i = 0; i < L; i++) {
		...
		if (c == 'D') {
			dir_change.push_back({ t,1 });
		}
		else {
			dir_change.push_back({ t,3 });
		}
	}
