...
void recur(int idx, int p, int f, int s, int v, int cost) {
	...
	/**
	 * 같은 비용의 집합이 하나 이상이면 사전 순으로 가장 빠른 것을 출력한다(재료를 선택하는 편이 사전 순이다). 예를 들어,
	 * 재료를 선택하는 경우의 수 [1 2 3 4 0], [0 0 0 0 5]가 최소 영양소 조건을 만족하면서 최소값이라면 [1 2 3 4 0]을 선택한다.
	 * [0 0 0 0 5]는 결국 temp 입장에서 5이기 때문에 후순위로 밀리게 된다
	 */
}
