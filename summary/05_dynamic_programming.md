...
1. 일단 recur로 작성
2. 시간 복잡도 통과 불가능? recur에서 dp[] 배열을 통해 값 저장해야겠다!
3. 어려운 dp의 경우 2번 과정 사이에 탑다운 / 바텀업 방식을 선택 후 진행한다.

<br>

### 쉬운 dp
1. 문제를 분석해서 점화식을 도출하자. 경험 상 dp[1], dp[2], dp[3]을 노가다로 구한 후 이들 간의 관계를 살펴보면 규칙을 발견하게 되고, 이를 점화식으로 표현하면 된다(자동으로 바텀업 타입이 된다).
2. 점화식을 기반으로 dp 테이블 내 값을 채운다. 특정 n에 대한 dp를 반환하면 된다.
...
- BOJ 14501 퇴사 : 날짜가 인덱스를 결정한다.<br><br>
    순수 recursive
    ```java
    import java.util.*;
    import java.io.*;
    ...
    public class Main {
        ...
        public static void main(String[] args) throws IOException {
            ...
            // 바텀업에서는 "이 칸이 계산되었는지"를 확인할 필요가 없으므로, -1 초기화 불필요
...
- BOJ 12865 평범한 배낭 : DP 작성시 신경쓸 변수 생각하기<br><br>
    ...
    public class Main {
        ...
        static int recur(int idx, int weight) {
            ...
            if (idx == n) { // idx는 항상 1씩 증가하므로 weight보다 후순위로 배치하는 것이 안전
                return 0;
            }
...
- BOJ 2240 자두나무 : boolean까지 생각하는 3차원 DP. 논리는 흐름 상 재귀 + dp 탑다운이 자연스럽다.